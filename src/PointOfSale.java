...
abstract class PointOfSale {
  ...
  /*
   * if
   * (System.getProperty("os.name").startsWith("W")||System.getProperty("os.name")
   * .startsWith("w")){
   * unixOS = false;
  ...
  public boolean enterItem(int itemID, int amount) // might include in a "mother class" in the future
  {
    ...
    for (int counter = 0; counter < databaseItem.size() && foundItem == false; counter++) {
      ...
      {
        transactionItem.add(
            new Item(itemID, databaseItem.get(counter).getItemName(), databaseItem.get(counter).getPrice(), amount));
        foundItem = true;
      }
    }
  ...
  public double updateTotal() {
     ...
     * transactionItem.get(counter).getItemID(),transactionItem.get(counter).
     * getItemName(),
     * transactionItem.get(counter).getAmount(),
     * transactionItem.get(counter).getPrice()*transactionItem.get(counter).
     * getAmount());
     */
  ...
  public void detectSystem() {
    if (System.getProperty("os.name").startsWith("W") || System.getProperty("os.name").startsWith("w")) {
      // unixOS = false; //these lines are commented out for running on netbeans,
      // which uses a linux protocol despite OS
      // couponNumber= "..\\Database\\couponNumber.txt";
      // tempFile="..\\Database\\temp.txt";
    }
  }
